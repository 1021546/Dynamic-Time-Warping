...
subplot(template_num+1, 2*template_num2, 4);
x = 1:length(y);
plot(x, y, 'k');
...
clear x;
subplot(template_num+1, 2*template_num2, 5);
...
for i2 = 1:template_num2
	for i = 1:template_num
		...
		% 畫圖
		subplot(template_num+1, 2*template_num2, (i2-1)*2+i*8+1);
		x = 1:length(y_t);
		plot(x, y_t, 'b');
		...
		clear x;
		subplot(template_num+1, 2*template_num2, (i2-1)*2+i*8+2);
